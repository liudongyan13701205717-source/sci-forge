"""分析学学科论文支持：实/复/泛函分析体裁、AMS 引用样式与估计式注记。"""

from __future__ import annotations

from sciforge.disciplines.base import Discipline

DISCIPLINE = Discipline(
    name="analysis",
    aliases=("analysis", "分析", "实分析", "复分析", "泛函分析", "调和分析",
             "real analysis", "complex analysis", "functional analysis",
             "harmonic analysis"),
    paper_types={
        "research": (
            "abstract",
            "introduction（背景、动机与主定理陈述）",
            "preliminaries（函数空间、范数与基本引理）",
            "main results（定理/命题，按编号陈述）",
            "proofs（证明，长证明可移附录）",
            "examples and applications（示例与应用）",
            "concluding remarks（进一步问题）",
            "references",
        ),
        "expository": (
            "abstract",
            "introduction",
            "background and motivation",
            "main exposition（以例子驱动的定理-证明链）",
            "exercises（练习，可选）",
            "references",
        ),
        "survey": (
            "abstract",
            "introduction",
            "historical overview（问题源流与关键节点）",
            "main developments（按主题组织的定理-证明链与归属）",
            "open problems（未解决问题清单）",
            "references",
        ),
    },
    citation_style="AMS 样式（作者-字母编号，amsrefs/BibTeX，如 [Smi04]）",
    reporting_standards={
        "proof": "每个定理必须给出完整证明，或明确引用出处；不得以“显然”替代论证",
        "attribution": "定理归属必须准确：首次证明者与其发表出处须在陈述或证明处引用",
        "regularity_conditions": "函数空间、可积性与光滑性假设须显式列出，并说明是否可放宽",
        "novelty_statement": "须明确区分本工作的结果与已有文献结果的边界",
    },
    conventions=(
        "定理环境按 theorem/lemma/proposition/corollary 分级编号（连号或分节）",
        "范数/内积/对偶配对记法全文一致：\\lVert \\cdot \\rVert、\\langle \\cdot , \\cdot \\rangle",
        "积分与测度记法统一：Lebesgue 积分用 \\int_X f \\, d\\mu，避免歧义",
        "证明以 QED 方块（\\qedsymbol）结束；嵌套证明标 (Proof of Lemma 2.3)",
        "常数 C 依赖的参数须显式标注（如 C(n,p)），并说明是否随维度退化",
        "假设与未证明的断言必须显式标注（conjecture/question）",
    ),
    key_venues=(
        "Journal of Functional Analysis",
        "Analysis & PDE",
        "Journal d'Analyse Mathématique",
        "Transactions of the American Mathematical Society",
        "Advances in Mathematics",
        "Duke Mathematical Journal",
    ),
    units_and_formulas_notes=(
        "使用 amsmath/amsthm/amssymb：对齐用 align/gather，多行推导按 = 号对齐",
        "新增算子用 \\DeclareMathOperator 声明（如 \\supp \\esssup \\dist），不手打 \\mathrm 拼算子",
        "估计式链用 \\lesssim 或显式常数，注明常数是否可计算",
        "显示公式仅在被正文引用时编号；行内公式避免复杂分式",
        "括号尺寸用 \\bigl \\bigr \\Bigl 系列而非手动放大",
    ),
    paper_capable=True,
    contribution_forms=("论文",),
    tools=("LaTeX", "MATLAB", "Mathematica", "Python (NumPy/SciPy)"),
    category="理学",
    databases=("arXiv", "OpenAlex", "Crossref"),
)